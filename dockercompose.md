# 🐳 Jeepay 项目 Docker 本地开发指南（团队版）

> 适用于 **Jeepay 本地开发 / 联调环境**
> 技术栈：**Colima + Docker + Docker Compose**

---

## 一、环境要求（必看）

| 工具             | 推荐版本    |
| -------------- | ------- |
| Docker         | 最新稳定版   |
| Docker Compose | v2+     |
| Colima（Mac）    | 最新      |
| JDK            | 与项目要求一致 |

> ⚠️ **Mac 用户统一使用 Colima，不直接使用 Docker Desktop**

---

## 二、启动 Docker（本地代理模式）

> 适用于：拉镜像、Maven / npm 依赖下载、访问外网资源

```bash
colima start \
  --env HTTP_PROXY=http://127.0.0.1:7890 \
  --env HTTPS_PROXY=http://127.0.0.1:7890 \
  --env NO_PROXY=localhost,127.0.0.1 \
  --cpu 4 \
  --memory 6
```

📌 **说明**

* 代理端口 `7890` 请根据个人代理软件调整
* 首次启动较慢，属正常现象

---

## 三、Docker Compose 启动方式（核心）

### 1️⃣ 启动服务（前台）

```bash
docker-compose up
```

### 2️⃣ 启动服务（后台，推荐）

```bash
docker-compose up -d
```

---

## 四、构建 & 重建策略（重点）

### 🔁 常规重新构建（推荐）

```bash
docker-compose up --build
```

适用场景：

* 代码更新
* Dockerfile 变更
* 依赖有调整

### 🔁 常规重新构建必须服务（不使用缓存）
```bash
docker-compose build --no-cache payment manager merchant
```

---

### ♻️ 强制重建容器（不重新 build 镜像）

```bash
docker-compose up --force-recreate
```

---

### 💥 完全重建（最彻底，慎用）

```bash
docker-compose up --build --force-recreate
```

⚠️ **注意**

* 容易产生 `<none>` 镜像
* 构建时间较长
* 问题严重时再使用

---

## 五、停止与清理服务

### ⏹️ 停止服务（不删容器）

```bash
docker-compose stop
```

---

### 🗑️ 停止并删除容器（保留数据卷）

```bash
docker-compose down
```

---

### 💣 停止并删除容器 + 数据卷（慎用）

```bash
docker-compose down -v
```

> ⚠️ 会删除 Redis / MySQL 数据，仅在数据可丢弃时使用

---

## 六、状态与日志排查

### 📊 查看服务运行状态

```bash
docker-compose ps
```

---

### 📄 查看指定服务日志

```bash
docker-compose logs payment
```

---

### 📡 实时查看日志（推荐）

```bash
docker-compose logs -f payment
```

---

## 七、进入容器排查问题

```bash
docker exec -it jeepay-payment bash
```

如容器内无 bash：

```bash
docker exec -it jeepay-payment sh
```

---

## 八、镜像清理规范（团队约定）

### 🧹 删除 jeepay 项目相关镜像

```bash
docker images | grep 'jeepay-' | awk '{print $3}' | xargs -r docker rmi -f
```

---

### 🧽 删除 `<none>` 悬空镜像（安全）

```bash
docker image prune -f
```

📌 建议：每周至少清理一次

---

## 九、常见坑 & 团队约定（必读）

### ❗ 容器端口规则（非常重要）

* **容器 ↔ 容器通信**：使用容器内部端口（如 Redis `6379`）
* **宿主机 ↔ 容器通信**：使用映射端口（如 `6380:6379`）

---

### ❗ Redis / MySQL 连接说明

* ❌ 不要在容器里使用 `localhost`
* ✅ 使用 **服务名**（如 `redis`、`mysql`）

---

### ❗ 镜像 Tag 规范（建议）

```text
jeepay-payment:1.0.0
jeepay-manager:1.0.0
```

避免长期使用 `latest`

---

## 十、推荐使用流程（新人必看）

```bash
# 1. 启动 Docker
colima start

# 2. 启动服务
docker-compose up -d

# 3. 查看状态
docker-compose ps

# 4. 查看日志
docker-compose logs -f payment
```

---

## 十一、统一故障排查流程

1. `docker-compose ps`
2. `docker-compose logs -f <service>`
3. `docker exec -it <container> bash`
4. 检查端口 / Redis / 数据库连接
5. 必要
